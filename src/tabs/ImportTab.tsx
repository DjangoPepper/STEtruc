...
export default function ImportPage() {
  ...
  useEffect(() => {
    ...
    setSplitFormats({});
    setMapping({
      rang:      headers.find((k) => /rang|n°|row|id|line|ligne/i.test(k)) ?? headers[0] ?? "",
      reference: headers.find((k) => /ref|coil|serial|coils|brames|bobine/i.test(k)) ?? (headers[1] ?? ""),
      poids:     headers.find((k) => /poids|weight|kg|ton(ne)|masse|bruto/i.test(k)) ?? (headers[2] ?? ""),
